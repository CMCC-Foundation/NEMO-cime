MODULE sbccpl_cesm
   !!======================================================================
   !!                       ***  MODULE  sbccpl_cesm  ***
   !! Surface Boundary Condition : NCAR CESM coupled formulation
   !!======================================================================
   !! History :  3.3  ! 2012 (P.G. Fogli, CMCC) Original code
   !!         :  4.2  ! 2025 (P.G. Fogli, CMCC) NEMO 4 update
   !!----------------------------------------------------------------------
...
CONTAINS
   ...
   SUBROUTINE sbc_cpl_cesm_rcv( kt )
      !! 
!-----------------------------------------------------------------------
!  This routine handle fields received from the CESM coupler
!
!    The following fields are always received from the coupler:
!
!    o  taux   -- zonal wind stress                        (W/m2   )
!    o  tauy   -- meridonal wind stress                    (W/m2   )
!    o  snow   -- water flux due to snow                   (kg/m2/s)
!    o  rain   -- water flux due to rain                   (kg/m2/s)
!    o  evap   -- evaporation flux                         (kg/m2/s)
!    o  meltw  -- sea ice/snow melt flux                   (kg/m2/s)
...
!    o  lwup   -- upward longwave radiation                (W/m2   )
!    o  lwdn   -- downward longwave radiation              (W/m2   )
!    o  melth  -- heat flux from sea ice/snow melt         (W/m2   )
!    o  ifrac  -- ice fraction
!    o  roff   -- liquid river runoff flux                 (kg/m2/s)
...
!-----------------------------------------------------------------------
      ...
      REAL(wp), DIMENSION(A2D(nn_hls)) ::   ztx, zty, work1
      ...
      !  1. Wind stress (momentum)
      !     rotate components from geo to local coordinates
      !     shift from T points to U,V points

      taux_x2o(:,:) = taux_x2o(:,:)*tmask(:,:,1)
      tauy_x2o(:,:) = tauy_x2o(:,:)*tmask(:,:,1)

      ! rotate true zonal/meridional wind stress onto local coordinates
      ...
      IF ( rn_rnf_bnd > 0.0_wp ) THEN
         ...
         work1(:,:) = max(rnf(:,:)-rn_rnf_bnd,0.0_wp)
         rnf(:,:)   = min(rnf(:,:),rn_rnf_bnd)
         zrnfex     = glob_sum('sbccpl_cesm', work1(:,:)*e1e2t(:,:))
      END IF

      ! emp: evap, liquid + solid precip and sea ice malting
      IF ( ln_rnf ) THEN
...
!         IF (lwp) WRITE(numout,*) ' RNF REDIST ',zrnfex
      ...
      ! 3. Salt Flux from cpl  [(Kg*psu)/(m^2 s)]
      sfx(:,:) = salt_x2o(:,:)
      ...
      !       Units: MKS
      work1(:,:) = rcp * emp(:,:) * sst_m(:,:)  ! emp contribution
      ! Net solar heat flux 
      qsr(:,:)   = swnet_x2o(:,:)
      ! Net non solar heat flux: long wave, sensible, latent, sea ice melting,
      ! latent heat of fusion of solid precip (snow & ice runoff), emp contribution
      qns(:,:)   = lwup_x2o(:,:)+lwdn_x2o(:,:)+sen_x2o(:,:)+lat_x2o(:,:) +   &
                   melth_x2o(:,:)-(snow_x2o(:,:)+ioff_x2o(:,:))*rLfus - work1(:,:)
      ...
      ! 6. atmospheric pressure and convert from Pa to hPa
      apr(:,:) = pslv_x2o(:,:)*0.01_wp*tmask(:,:,1)
